...
def custom_draw_geometry(pcd,linesets_cpp):
...
def view_res(view_range):
    ...
    for i in range(100):
        print(i)
        points = np.load("/home/cxy/Documents/CPP_ALG/second_cpp-32line/dataset/gate8/" + str(i).zfill(4) + ".npy");
        bbox_cpp = np.load("/home/cxy/Documents/CPP_ALG/second_cpp-32line/dataset/gate_8_res_0810/" + str(i).zfill(4) + "_cpp.npy")
        ...
        custom_draw_geometry(point_cloud,line_sets_cpp)

def view_pc(pc_path):
    points = np.load("1_cpp_res.npy")
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(points[:, :3])
...
def view_cluster_res(pc_path):
    points = np.load(pc_path)
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(points[:, :3])
    point_cloud.colors = o3d.utility.Vector3dVector(points[:, 4:7])
...
if __name__ == "__main__":
    view_cluster_res("/home/cxy/CLionProjects/dbscan/point_data/1_cpp_dbscan.npy")
